...
export const DEFAULT_MESSAGES = {
  ...
  errors: {
    ...
    no_permission_session:
      '❌ **You do not have permission to interact with this diagnostic session.**',
  ...
  setup: {
    configure_select_repo:
      '📋 **Configure Jules Diagnostic Session**\nPlease select the repository you want to run diagnostics against:',
    configure_select_branch:
      '📋 **Configure Jules Diagnostic Session**\nSelected Repository: `{repo}`\nPlease select the branch to work on:',
    configure_select_branch_search:
      '📋 **Configure Jules Diagnostic Session**\nSelected Repository: `{repo}` (Search results for: `{query}`)\nPlease select the branch to work on:',
    no_connected_repos:
      '⚠️ **No connected repositories found in your Jules account.** Please connect a repository first.',
    load_repos_failed:
      '❌ **Failed to load connected repositories for selection.** Please verify your connection to Google Jules and try again.',
    no_default_repo:
      '⚠️ **No default repository has been set for this server.** Please use the `/link-repo` command to set a default repository.',
    no_branches_matched:
      '❌ **No branches matched your search query "{query}".** Please try again.',
  ...
  plan: {
    ...
    approved_via_command:
      '✅ **Plan approved via slash command! Jules is continuing the diagnostic steps...**',
    rejected:
  ...
  commands: {
    ...
    approve_cannot_approve_state:
      '❌ **Cannot approve plan.** Current session state is `{state}` (needs to be `awaitingPlanApproval`).',
    approve_failed:
      '❌ **Failed to approve plan. An error occurred while communicating with Jules.**',
    link_repo_description:
      'Link a GitHub repository to this server as the default for Jules diagnostic sessions',
    link_repo_option_description: 'GitHub repository in owner/repo format (e.g. facebook/react)',
    link_repo_invalid_format:
      '❌ Invalid repository format. Please use `owner/repo` format (e.g., `facebook/react`).',
    link_repo_success:
      '✅ **Successfully linked repository `{repo}` to this server!** Jules will now analyze this repository for new debug threads.',
    link_repo_failed: '❌ Failed to link repository in the database.',
    setup_forum_description:
      'Set the designated Forum channel where Jules will monitor debug threads',
